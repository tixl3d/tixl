...
// Stroke segments use screen coordinates; recorded hits and anchor placements use canvas coordinates.
internal sealed class ConnectionStroke
{
    ...
    // Invoked before PathStroke consumes the pending path; observation must leave that path intact.
    internal Action<ImDrawListPtr> ObservePath { get; }
    internal bool IsActive { get; private set; }
    internal bool IsCut { get; private set; }

    // position is in screen coordinates; cut selects deletion instead of reroute insertion.
    ...
    // position and dragThreshold are in screen pixels.
    ...
    // The canvas must release the borrowed wire at the end of the draw pass, including on failure.
    ...
    // The caller must end the gesture even when committing fails.
    ...
    // Coordinates and tolerance are in screen pixels; crossing lies on the cable, including for near misses.
    ...
    // Preview and commit share these cached canvas-space placements.
