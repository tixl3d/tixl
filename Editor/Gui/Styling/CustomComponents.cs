...
internal static partial class CustomComponents
{
    ...
    /// <summary>
    /// A column of fixed-width select buttons, one per enum value — the category selector of a settings
    /// window. The horizontal pill is <see cref="SegmentedButton{T}"/>; the two are separate controls, not
    /// modes of one.
    /// </summary>
    public static bool SelectButtonColumn<T>(ref T selectedValue, float width) where T : struct, Enum
    {
        var modified = false;
        ...
        foreach (var value in Enum.GetValues<T>())
        {
            ...
            if (DrawSelectButton(name, isSelected, width))
            {
                modified = true;
                selectedValue = value;
            }
