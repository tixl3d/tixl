...
internal static partial class PlayerExporter
{
                                          ...
                                          [NotNullWhen(true)] out string? address)
    {
        ...
        if (playbackSettings.TryGetMainSoundtrack(instance, out var soundtrack) is not true)
        {
            ...
            {
                Log.Warning($"You should define soundtracks withing the exported operators. Falling back to {soundtrack.Clip.Address} set in parent...");
            }
            else
            {
                address = null;
        ...
        address = soundtrack.Clip.Address;
        return FileResource.TryGetFileResource(soundtrack.Clip.Address, instance, out _);
    }
