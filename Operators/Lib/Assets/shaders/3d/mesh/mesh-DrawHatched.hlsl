...
float4 psMain(psInput pin) : SV_TARGET
{
    ...
    float brightness = ((directLighting.r + directLighting.g+ directLighting.b)/3 + randFaceOffset * RandomFaceLighting).x;
    ...
    float hatch = saturate( (sin(p.x / (LineWidth * RequestedResolutionHeight / 500 /  (2 * PI))) + (shadingOffset * 3 -1)) * 1).x;
