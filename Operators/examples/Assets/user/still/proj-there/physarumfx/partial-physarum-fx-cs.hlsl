...
{   
    ...
    float _rotateToComfort = CB.RotateToComfort + (float)(block.x - BlockCount.x/2) * 0.1 + (fxTexture.r -0.5) * FXRedFor_Rotate;
    ...
    float _baseMove = CB.BaseMovement + ((float)block.y - BlockCount.x/2.) * 5;
