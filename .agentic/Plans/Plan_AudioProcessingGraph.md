...
3. **FFT / audio-reactive — becomes a graph tap, no designation needed.** Default: analyze the **final mix** (react to what you hear). Advanced: an `[AudioAnalysis]`-style op taking an `AudioReference` to meter a specific bus (e.g. soundtrack-only, excluding SFX). Adjacent to G5 `[AudioLevel]`/`[Duck]`.
