...
- **Done (2026-06-14): index button.** The selector-bar index is now a rounded button (hover-lit) with the zero-padded index, tooltip "Click to edit MIDI controller indices".
- **Deferred:** optional **thumbnail cells**; persisting the chosen layout (currently session-only) and a per-device layout for the non-APC controllers.
